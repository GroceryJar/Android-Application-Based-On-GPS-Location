...
public class NearByFragment extends Fragment  {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		loc = new GPSTracker(getActivity());
		if(loc.canGetLocation())
		{
			...
			products = user.getNearByList(0,10,loc.getLatitude(),loc.getLongitude());
			...
			listview.setOnScrollListener(new OnScrollListener() {
				...
				public void onScroll(AbsListView view,int firstVisibleItem, int visibleItemCount,int totalItemCount) {
					...
					if(lastItem == totalItemCount){
						if(moreProduct)
						{
							products = user.getNearByList(totalItemCount,totalItemCount+10,loc.getLatitude(),loc.getLongitude());
							try {
								if(products.getInt("success")==1)
								{
									total = products.getInt("total");
